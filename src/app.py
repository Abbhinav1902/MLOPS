...
cursor = conn.cursor()
cursor.execute("CREATE TABLE IF NOT EXISTS logs (input TEXT, prediction INTEGER)")
conn.commit()

prediction_counter = Counter("iris_predictions_total", "Total number of predictions", ["prediction"])
...
def predict():
    try:
        ...
        cursor.execute("INSERT INTO logs (input, prediction) VALUES (?, ?)", (str(data.dict()), int(prediction)))
        conn.commit()
